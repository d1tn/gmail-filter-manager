...
function generateFromConditionXML(fromConditions) {
    ...
    if (fromConditions.length > 1) {
        // OR条件グループをフォーマット
        const fromParts = fromConditions.map(orGroup => {
            // ANDキーワードを除去して実際の値だけを取得
            const values = orGroup.filter(item => item !== 'AND');
            if (values.length === 1) {
                // 単一値の場合はそのまま
                return `from:${escapeXml(values[0])}`;
            } else {
                // 複数値（AND条件）の場合は括弧でグループ化
                const andCondition = values.map(v => `from:${escapeXml(v)}`).join(' AND ');
                return `(${andCondition})`;
            }
        });

        // すべてのOR条件を組み合わせる
        const combinedQuery = fromParts.join(' OR ');
        xml += `    <apps:property name="hasTheWord" value="${escapeXml(combinedQuery)}"/>\n`;
    } else if (fromConditions.length === 1) {
        // 単一のORグループの場合（シンプルなfrom条件）
...
function generateDoesNotHaveTheWordConditionXML(excludesConditions) {
    ...
    // 複数のORグループがある場合は複合条件として処理
    // 「含まない」条件ではOR関係は複雑なため、すべてを個別のdoesNotHaveTheWord条件として扱う
    if (excludesConditions.length > 0) {
        // 各ORグループを個別の条件としてフォーマット
        excludesConditions.forEach(orGroup => {
            // ANDキーワードを除去して実際の値だけを取得
            const values = orGroup.filter(item => item !== 'AND');
            ...
            if (values.length === 1) {
                ...
                // 複数キーワード（AND条件）の場合
                // 「含まない」のAND条件は複雑なため、個別の条件として扱う
                values.forEach(value => {
                    xml += `    <apps:property name="doesNotHaveTheWord" value="${escapeXml(value)}"/>\n`;
                });
            }
        });
    }
